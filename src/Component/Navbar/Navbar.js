                ...
                <div className={`${isOpen ? 'translate-x-0 opacity-95 ' : 'opacity-0 -translate-x-full '}absolute inset-x-0 z-20 w-full px-6 py-4 transition-all duration-300 ease-in-out bg-white dark:bg-gray-800 md:mt-0 md:p-0 md:top-0 md:relative md:bg-transparent md:w-auto md:opacity-100 md:translate-x-0 md:flex md:items-center`}>
                    <div className="flex flex-col md:flex-row justify-items-center md:mx-6">
...

                        {
                            user ?
                                <>
                                    <NavLink className={({ isActive }) => isActive ? 'text-primary md:mx-4 md:my-0 my-2 text-lg font-medium' : "my-2 text-lg font-medium text-gray-800 transition-colors duration-300 transform dark:text-gray-200 hover:text-primary  md:mx-4 md:my-0"} to={'/dashboard'}>Dashboard</NavLink>
                                    <NavLink onClick={handleLogOut} className="my-2 text-lg font-medium  transition-colors duration-300 transform text-gray-800 hover:text-maroon dark:hover:text-blue-400 md:mx-4 md:my-0" >Log Out</NavLink></> :
                                <NavLink className={({ isActive }) => isActive ? 'text-primary md:mx-4 md:my-0 my-2 text-lg font-medium' : "my-2 text-lg font-medium  transition-colors duration-300 transform text-gray-800 hover:text-primary  md:mx-4 md:my-0"} to={'/login'}>Login</NavLink>
                        }
                    </div>
                    ...
                    <div className="flex justify-center md:block">
                        {user && <Link className="relative text-gray-700 transition-colors duration-300 transform dark:text-gray-200 hover:text-gray-600 " href="#">
                            <div className="relative">
                                {user.photoURL && <img className="object-cover w-10 h-10 rounded-full ring-2 ring-primary dark:ring-primary" src={user.photoURL} alt="" />
                    ...
                    <Link className="block px-5 py-2 mt-4 text-sm text-center text-white capitalize bg-primary rounded-lg lg:mt-0 hover:bg-secondary lg:w-auto" href="#">
                        Get started
                    </Link>
                </div>
            </div>
